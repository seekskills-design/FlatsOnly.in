...
export default function EditProperty() {
    ...
    const fetchProperty = async () => {
      ...
      try {
        ...
        if (docSnap.exists()) {
          ...
          // Parse location into locality and city if possible
          let locality = "";
          let city = "";
          if (data.location) {
          ...
          setFormData({
            ...
            maintenanceType: data.maintenanceType || "Per Month",
  ...
  const handleSubmit = async () => {
    ...
    try {
      // Compress new images and get base64 strings
      ...
      try {
        ...
        alert("Image processing failed. Submitting property without new images.");
      ...
      const updateDocPromise = updateDoc(doc(db, "properties", id), propertyData);
      const dbTimeoutPromise = new Promise((_, reject) => {
        setTimeout(() => reject(new Error("Database timeout: Could not save property. Please check your connection.")), 10000);
      });

      await Promise.race([updateDocPromise, dbTimeoutPromise]);
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      <SEO title="Edit Property" description="Edit your existing flat listing on FlatsOnly." noindex={true} />
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-white tracking-tight">Edit Property</h1>
          ...
          {step === 6 && (
            <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
              ...
              {(existingImages.length > 0 || previewUrls.length > 0) && (
                <div className="mt-6">
                  <h4 className="text-sm font-medium text-gray-700 mb-3">Images ({existingImages.length + previewUrls.length})</h4>
                  <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
                    ...
                    {previewUrls.map((url, index) => (
                      <div key={`new-${index}`} className="relative group rounded-xl overflow-hidden border border-gray-200 aspect-square">
